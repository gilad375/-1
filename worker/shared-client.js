(() => {
  ...
  window.submitRecipe = async () => {
    ...
    const recipe = {
      ...
      kidsFriendly: document.getElementById("recipeKidsFriendly")?.checked || false,
      goldenRecipe: document.getElementById("recipeGolden")?.checked || false,
      secretIngredient: document.getElementById("recipeSecretIngredient")?.value.trim() || "",
      equipment: document.getElementById("recipeEquipment")?.value.trim() || "",
      tasteProfile: document.getElementById("recipeTasteProfile")?.value.trim() || "",
    };
  ...
  window.saveEditedRecipe = async () => {
    ...
    const recipe = {
      ...
      kidsFriendly: document.getElementById("editRecipeKidsFriendly")?.checked || false,
      goldenRecipe: document.getElementById("editRecipeGolden")?.checked || false,
      secretIngredient: value("editRecipeSecretIngredient").trim(), equipment: value("editRecipeEquipment").trim(), tasteProfile: value("editRecipeTasteProfile").trim(),
    };
  ...
  function renderRecipeExtras() {
    ...
    if(recipe.equipment||recipe.secretTip||recipe.tasteProfile||recipe.cooks?.length||Object.values(recipe.taste||{}).some(Number)) {const familyNote=node("section","recipeFamilyNotes");familyNote.append(node("h3","","📖 הסיפור המשפחתי למתכון"));if(recipe.equipment)familyNote.append(node("p","",`🍳 ציוד: ${recipe.equipment}`));if(recipe.secretTip)familyNote.append(node("p","",`✨ הטיפ הסודי של המשפחה: ${recipe.secretTip}`));if(recipe.cooks?.length)familyNote.append(node("p","",`👩‍🍳 בישלו יחד: ${recipe.cooks.map(c=>c.task?`${c.member} — ${c.task}`:c.member).join(" · ")}`));const tastes=[["sweet","מתוק"],["salty","מלוח"],["spicy","חריף"],["sour","חמוץ"]].filter(([key])=>Number(recipe.taste?.[key])>0);if(tastes.length)familyNote.append(node("small","",`👅 מד הטעם: ${tastes.map(([key,label])=>`${label} ${recipe.taste[key]}/5`).join(" · ")}`));else if(recipe.tasteProfile)familyNote.append(node("small","",`👅 מד הטעם: ${recipe.tasteProfile}`));extra.appendChild(familyNote)}
  ...
  window.editCurrentRecipe = function (...args) {
    ...
    const check = (field, value) => { const el = document.getElementById(field); if (el) el.checked = Boolean(value); };
    check("editRecipeKidsFriendly", window.currentRecipe?.kidsFriendly);
    check("editRecipeGolden", window.currentRecipe?.goldenRecipe);
    set("editRecipeSecretIngredient", window.currentRecipe?.secretIngredient);
    set("editRecipeEquipment", window.currentRecipe?.equipment);
    set("editRecipeTasteProfile", window.currentRecipe?.tasteProfile);
    return result;
  };
